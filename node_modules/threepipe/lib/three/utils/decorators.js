import { FnCaller, getOrCall, objectHasOwn, safeSetProperty } from 'ts-browser-helpers';
/**
 *
 ...
 */
export function uniform({ uniforms, propKey, thisTarget = false, onChange } = {}) {
    ...
    return (targetPrototype, propertyKey, descriptor) => {
        const getUniform = (target) => {
        ...
        if (descriptor) {
            ...
            if (objectHasOwn(descriptor, 'writable'))
                delete descriptor.writable;
            if (objectHasOwn(descriptor, 'initializer'))
 ...
 */
export function matDefine(key, customDefines, thisMat = false, onChange, processVal, invProcessVal) {
    // backing up properties as values are different when called again, no idea why.
    ...
    return (targetPrototype, propertyKey, descriptor) => {
        ...
        const prop = {
            get() {
                ...
                let res = t[p];
                if (invProcessVal)
                    res = invProcessVal(res);
                return res;
            },
            set(newVal) {
                const { t, p } = getTarget(thisMat ? this : this.material);
                if (processVal)
                    newVal = processVal(newVal);
 ...
 */
export function matDefineBool(key, customDefines, thisMat = false, onChange, deleteOnFalse = false) {
    // noinspection RedundantConditionalExpressionJS
    return matDefine(key, customDefines, thisMat, onChange, (v) => v ? '1' : deleteOnFalse ? undefined : '0', (v) => v && v !== '0' ? true : false);
}
/**
 ...
 * @param invProcessVal - function that processes the value before returning it.
 */
export function bindToValue({ obj, key, processVal, invProcessVal, onChange, onChangeParams = true }) {
    const cPropKey = !!key;
    return (targetPrototype, propertyKey, descriptor) => {
        const getTarget = (_this) => {
        ...
        const prop = {
            ...
            set(newVal) {
                ...
                if (newVal === undefined)
                    delete t[p];
                let oc = onChange;
                if (oc && (typeof oc === 'string' || typeof oc === 'symbol'))
                    oc = this[oc]; // todo just call it here directly
                if (oc && typeof oc === 'function')
                    FnCaller.callFunction(oc, this, onChangeParams ? [p, newVal] : []);
            },
            // configurable: true,
            // enumerable: true,
        };
