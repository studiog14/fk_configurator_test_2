...
export function overrideThreeCache(storage) {
    ...
    threeCache.add = async (url, data, responseType) => {
        if (!responseType)
            return oldCache.add(url, data);
        if (url.startsWith('data:') || url.startsWith('blob') || url.startsWith('chrome-extension'))
            return;
        // noinspection JSIgnoredPromiseFromCall
        if (await storage.match(url))
            await storage.delete(url);
        await storage.put(url, new Response(data, { status: 200 }));
    };
    threeCache.remove = (url, responseType) => {
        if (!responseType)
            return oldCache.remove(url);
        // noinspection JSIgnoredPromiseFromCall
        storage.delete(url);
    };
}
